...
Private methods cannot be invoked with an explicit caller. On `line 56` we are invoking `serial_number` with an explicit caller `other` which raises `NoMethodError`.

We need to change `serial_number` to a protected method. From outside the class protected methods act just like private methods, but from inside the class they can be invoked with an explicit caller.