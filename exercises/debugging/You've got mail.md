...
At the moment we are invoking `Object#send` method, which takes the name of the method we want to call as the first argument, and arguments we want to pass into that method as the following arguments). Which means we are trying to invoke method with name ``860 Blackbird Ln.'` and it doesn't exist.

We are really not looking for this functionality in this case. The functionality we are looking for is defined in `Mailing` module that we haven't mixed in our class.
...
We should also rename our `send` method to something unique like `send_mail` to avoid accidental method overriding like above.