...
# - This will expose `@quantity` instance variable through setter method `quantity=`
#   Now, we can just call `quantity=` on InvoiceEntry object and update @quantity to any value, even negative,
#   essentially circumventing the `update_quantity` method and it's `if updated_count >= 0` guard clause
